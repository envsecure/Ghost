...
async function addSubscription(email: string, overrides: SubscriptionOverrides) {
    return StripeCustomerSubscription.add({
        ...baseSub,
        // current_period_end is required by the schema but isn't part of the
        // resolution rule — give every test sub a sensible default in the
        // future so it survives DB validation.
...
describe('Subscription resolution priority', function () {
    ...
    it('breaks identical-start_date ties deterministically by subscription id', async function () {
        const member = await createMemberWithCustomer('twins@test.member');

        const sharedStart = daysAgo(10);
